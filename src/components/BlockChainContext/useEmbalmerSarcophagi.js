...
const useEmbalmerSarcophagi = (sarcophagusContract) => {
  ...
  const getSarcophagiDoubleHashes = useCallback( async (count) => {
    try {
      ...
      for(let i = 0; i <= count - 1; i++) {
        const doubleHash = await sarcophagusContract.embalmerSarcophagusDoubleHash(account, i)
  ...
  const getSarcophagInfo = useCallback(async () => {
    try {
      const embalmerSarcophagi = await Promise.all(sarcoDoubleHashes.map( async (doubleHash) => {
        return {
  ...
  return { embalmerSarcophagi, pendingCount }
}
